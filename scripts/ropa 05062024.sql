...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 05-06-2024 a las 23:34:00
...
--
-- Estructura Stand-in para la vista `marcas_con_almenos_una_venta`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `marcas_con_almenos_una_venta` (
...
(4, 4, 'Marmont', 'Bolsos', 'Rojo', 'Única', 800, 30),
...
--
-- Estructura Stand-in para la vista `prendas_vendidas_y_cantidad_restante_en_stock`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `prendas_vendidas_y_cantidad_restante_en_stock` (
`idPrenda` int(11)
,`nombrePrenda` varchar(50)
,`stockRestante` int(11)
...
--
-- Estructura Stand-in para la vista `top_5_marcas_vendidas_y_cantidad_vendida`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `top_5_marcas_vendidas_y_cantidad_vendida` (
`nombreMarca` varchar(50)
,`totalVendidas` decimal(32,0)
);
...
--
-- Estructura para la vista `marcas_con_almenos_una_venta`
--
DROP TABLE IF EXISTS `marcas_con_almenos_una_venta`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `marcas_con_almenos_una_venta`  AS SELECT `m`.`nombre` AS `nombreMarca`, sum(`v`.`cantidadVendida`) AS `totalVendido` FROM ((`marcas` `m` join `prendas` `p` on(`m`.`idMarca` = `p`.`idMarca`)) join `ventas` `v` on(`p`.`idPrenda` = `v`.`idPrenda`)) GROUP BY `m`.`nombre``nombre`  ;

-- --------------------------------------------------------

--
-- Estructura para la vista `prendas_vendidas_y_cantidad_restante_en_stock`
--
DROP TABLE IF EXISTS `prendas_vendidas_y_cantidad_restante_en_stock`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `prendas_vendidas_y_cantidad_restante_en_stock`  AS SELECT `p`.`idPrenda` AS `idPrenda`, `p`.`nombre` AS `nombrePrenda`, `p`.`stock` AS `stockRestante`, ifnull(sum(`v`.`cantidadVendida`),0) AS `cantidadVendida` FROM (`prendas` `p` left join `ventas` `v` on(`p`.`idPrenda` = `v`.`idPrenda`)) GROUP BY `p`.`idPrenda`, `p`.`nombre`, `p`.`stock``stock`  ;

-- --------------------------------------------------------

--
-- Estructura para la vista `top_5_marcas_vendidas_y_cantidad_vendida`
--
DROP TABLE IF EXISTS `top_5_marcas_vendidas_y_cantidad_vendida`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `top_5_marcas_vendidas_y_cantidad_vendida`  AS SELECT `m`.`nombre` AS `nombreMarca`, sum(`v`.`cantidadVendida`) AS `totalVendidas` FROM ((`marcas` `m` join `prendas` `p` on(`m`.`idMarca` = `p`.`idMarca`)) join `ventas` `v` on(`p`.`idPrenda` = `v`.`idPrenda`)) GROUP BY `m`.`idMarca`, `m`.`nombre` ORDER BY sum(`v`.`cantidadVendida`) DESC LIMIT 0, 55  ;
